# Daily AI Brief Generator - PowerShell Version
# 생성형 AI 뉴스레터 자동 생성 스크립트

$ErrorActionPreference = "Stop"
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$WorkspaceDir = Split-Path -Parent $ScriptDir
$ConfigFile = Join-Path $WorkspaceDir "config\news_sources.json"
$OutputDir = Join-Path $WorkspaceDir "daily_ai_brief"
...
Write-Host "🤖 Daily AI Brief Generator 시작..."
...
if (Test-Path $ConfigFile) {
    ...
    Write-Host "✅ 설정 파일 로드 완료"
} else {
    Write-Host "⚠️  설정 파일을 찾을 수 없습니다: $ConfigFile"
}
...
Write-Host "✅ $($articles.Count)개 기사 수집 완료"
...
# HTML 생성
$today = Get-Date
$dateStr = $today.ToString("yyyy년 MM월 dd일")
$fileDateStr = $today.ToString("yyyy-MM-dd")
...
<head>
    ...
    <title>Daily AI Brief - $dateStr</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
# 카테고리별 섹션 추가
$categoryOrder = @('속보', '주요 뉴스', '기술 트렌드', '산업뉴스', '기타')
$emojiMap = @{
    '속보' = '🔴'
    '주요 뉴스' = '📰'
    '기술 트렌드' = '🚀'
...
foreach ($category in $categoryOrder) {
    if ($byCategory[$category]) {
        ...
        $html += "                <h2 class=`"section-title`">$emoji $category</h2>`n"

        foreach ($article in $byCategory[$category]) {
            $html += @"
                <div class="article">
...
Write-Host "✅ HTML 생성 완료"

# 파일 저장
$filePath = Join-Path $OutputDir "$fileDateStr.html"
$html | Out-File -FilePath $filePath -Encoding UTF8
Write-Host "✅ 파일 저장: $filePath"

# 인덱스 생성
$indexPath = Join-Path $OutputDir "index.html"
$htmlFiles = @(Get-ChildItem -Path $OutputDir -Filter "*.html" -Exclude "index.html" | Sort-Object -Property Name -Descending)
...
if ($htmlFiles.Count -gt 0) {
    $latestFile = $htmlFiles[0]
    $latestDate = [System.IO.Path]::GetFileNameWithoutExtension($latestFile.Name)

    $indexHtml += @"
        <div class="link-section">
            <p>📰 <strong>최신 뉴스레터</strong></p>
            <a href="$($latestFile.Name)">📄 $latestDate 뉴스레터 보기</a>
        </div>
"@
}
...
"@
    ...
    $htmlFiles | Select-Object -First 10 | ForEach-Object {
        ...
        $indexHtml += "            <a href=`"$($_.Name)`">$name</a><br>`n"
    }
...
Write-Host "✅ 인덱스 생성 완료"

Write-Host ""
Write-Host "📧 Daily AI Brief 생성이 완료되었습니다!"
